...
class AnalyticsService:
    ...
    async def get_total_negative_accounts_count(self, account) -> int:
        if account.role not in ADMIN_ROLES:
            raise NotSufficientPrivileges()
        cycle_ids = await self.repository.get_cycle_ids_for_today()
        accounts = await self.repository.get_accounts_with_negative_balance_for_cycle_ids(cycle_ids=cycle_ids)
        return len(accounts)

    async def get_analytics_data_for_today(self, account):
        if account.role not in ADMIN_ROLES:
            raise NotSufficientPrivileges()
        total_sum = self.get_total_sum_earned(account)
        negative_accounts_count = self.get_total_negative_accounts_count(account)
    ...
    async def get_most_expensive_task_cost_for_period(self, account, date_from, date_till):
        if account.role not in ADMIN_ROLES:
            raise NotSufficientPrivileges()
        most_expensive_task_for_period = self.repository.get_most_expensive_task_cost_for_period(date_from, date_till)
