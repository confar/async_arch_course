...
async def assign_tasks(user=Depends(get_current_user),
    ...
    try:
        task_service.assign_tasks(request_user=user)
...
async def create_task(body: AddTaskSerializer,
                      ...
                      task_service=Depends(get_task_service)):
    task = task_service.create_task(creator_id=user.id, description=body.description, assignee_id=body.assignee_id)
    return task


@router.get("/my-tasks/", response_model=TaskSerializer)
async def get_my_tasks(user=Depends(get_current_user),
                       task_service=Depends(get_task_service)):
    tasks = task_service.get_user_tasks(worker_id=user.id)
    return tasks
...
async def mark_task_done(user=Depends(get_current_user),
                         ...
                         task_service=Depends(get_task_service)):
    try:
        task = task_service.complete_task(worker=user, task_id=task.id)
